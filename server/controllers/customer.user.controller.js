...
exports.removeCustomerUser = (req, res, next) => {
  customerUserService
    .removeCustomerUser(req.params.id)
    .then(data => res.status(data.status).json(data))
    .catch(err => res.status(err.status).json(err));
};
// update a customer
  ...
  if (req.body.password) {
    bcrypt.genSalt(config.salt_rounds, (err, salt) => {
      bcrypt.hash(req.body.password.trim(), salt, (err, hash) => {
        req.body.password = hash;
        customerUserService
          .updateCustomerUser(req.params.id, req.body)
          .then(data => res.status(data.status).json(data))
          .catch(err => res.status(err.status).json(err));
      });
    });
...
exports.getCustomerUserByName = (req, res, next) => {
  customerUserService
    .getCustomerUsersByName(req.params.name)
    .then(data => res.status(data.status).json(data))
    .catch(err => res.status(err.status).json(err));
};
// get customer by id
...
exports.getCustomerUsersByPhone = (req, res, next) => {
  customerUserService
    .getCustomerUsersByPhone(req.params.phone)
    .then(data => res.status(data.status).json(data))
    .catch(err => res.status(err.status).json(err));
};
...
exports.addMultipleCustomerUsers = (req, res, next) => {
  ...
  if (failed.length > 0) {
    ...
    customerUserService
      .addMultipleCustomerUsers(succeeded)
      .then(data => {
        console.log(data);
